...
<div class="form-page">
    ...
    <div class="page-header mb-4">
        <a href="{{ route('assignments.show', [$course, $assignment]) }}" class="btn btn-light rounded-circle me-3">
            <i class="bi bi-arrow-left"></i>
        </a>
        <div>
            <h2 class="mb-1 fw-bold">Edit Tugas</h2>
            <p class="text-muted mb-0 small">{{ $course->title }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('assignments.update', [$course, $assignment]) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-card mb-3">
            <div class="form-card-body">
                ...
                <input type="number" name="order" class="form-control form-control-lg @error('order') is-invalid @enderror" 
                       value="{{ old('order', $assignment->order) }}" 
                       min="1" required>
                @error('order')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
        ...
        <div class="form-card mb-3">
            <div class="form-card-body">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                          rows="6" placeholder="Jelaskan tugas yang harus dikerjakan siswa...">{{ old('description', $assignment->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="form-card mb-3">
            <div class="form-card-body">
                <label class="form-label fw-semibold">File yang Ada</label>
                    ...
                    @foreach($assignment->attachments as $index => $attachment)
                        <div class="file-item">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="keep_attachments[]" 
                                       value="{{ $index }}" id="keep{{ $index }}" checked>
                ...
                <small class="text-muted">Uncheck untuk menghapus file</small>
            </div>
        </div>
        @endif
        ...
        <div class="form-card mb-4">
            <div class="form-card-body">
                <label class="form-label fw-semibold">Tambah File Baru</label>
                ...
                <small class="text-muted">Opsional - Upload file pendukung tugas tambahan</small>
            </div>
        </div>
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('assignments.show', [$course, $assignment]) }}" class="btn btn-outline-secondary btn-lg rounded-pill flex-fill">
...
.existing-files .form-check-input {
    width: 20px;
    height: 20px;
    margin: 0;
    flex-shrink: 0;
...
.existing-files .form-check-label {
    display: flex;
    align-items: center;
    gap: 12px;
    flex-grow: 1;
